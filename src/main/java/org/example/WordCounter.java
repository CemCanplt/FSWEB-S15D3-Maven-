...
public class WordCounter {
    ...
    public static Map<String, Integer> calculatedWord() {
        ...
        Map<String, Integer> uniqueKelime = new HashMap<>();

        String[] words = text.toLowerCase().replaceAll("[^a-zA-Z0-9şöçğüıİ]+", " ").split("\\s+");

        for (String word: words) {
            uniqueKelime.put(word, uniqueKelime.getOrDefault(word, 0) + 1);
        }
